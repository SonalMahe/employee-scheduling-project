...
const EmployeeAvailability = () => {
  ...
  const getStatusInfo = (dayInfo, shift) => {
    const key = `${dayInfo.dayEnum}-${SHIFT_MAP[shift]}`;
    const status = availability[key];
    if (status === "AVAILABLE") return { className: "cell available", label: "Available" };
    if (status === "UNAVAILABLE") return { className: "cell unavailable", label: "Unavailable" };
    if (status === "PREFERRED") return { className: "cell prefer", label: `Preferred ${SHIFT_TIMES[shift]}` };
    return { className: "cell", label: null };
  };
          ...
          <tbody>
            {shifts.map((shift) => (
              <tr key={shift}>
                ...
                {weekDays.map((d) => {
                  const info = getStatusInfo(d, shift);
                  return (
                    <td key={d.dayEnum} className={info.className}>
                      <button
                        className={`choose-btn ${info.label ? 'has-status' : ''}`}
                        onClick={() => openModal(d, shift)}
                      >
